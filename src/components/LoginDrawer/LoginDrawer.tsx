...
export function LoginDrawer({
  ...
  return (
    <div className={`flex flex-col bg-surface-page ${sizeStyles[size]} ${className}`.trim()}>
      <button
        onClick={onClose}
        className={`self-end ${closeIconSize} flex items-center justify-center`}
        aria-label="Close"
      >
        <img src={closeIcon} alt="" className={`${closeIconSize}`} aria-hidden="true" />
      </button>

      <div className={`flex flex-col ${formGapStyles[size]}`}>
        <div className="flex flex-col gap-8">
          <h2 className={`font-heading font-medium text-brand ${titleStyles[size]}`}>
